...
function makeRun(overrides: Partial<ConformanceRun> = {}): ConformanceRun {
  const base: ConformanceRun = {
    ...
    finishedAt: 1000,
    checks: [
      { id: 'manifest/event-kind', area: 'manifest', severity: 'error', title: 'type', status: 'pass' },
      { id: 'wire/envelope-well-formed', area: 'wire', severity: 'error', title: 'wire', status: 'skip', detail: 'no envelopes' },
      { id: 'lifecycle/clean-teardown', area: 'lifecycle', severity: 'warning', title: 'teardown', status: 'fail', detail: 'listener leak' },
    ],
...
describe('toPretty', () => {
  it('renders the napplet name, statuses, and a conformant result', () => {
    ...
    expect(text).toContain('[PASS] manifest/event-kind');
  ...
  it('renders a non-conformant result with the error count', () => {
    const run = makeRun({
      ...
      checks: [
        { id: 'manifest/event-kind', area: 'manifest', severity: 'error', title: 'type', status: 'fail', detail: 'missing' },
        { id: 'lifecycle/clean-teardown', area: 'lifecycle', severity: 'warning', title: 'd', status: 'fail' },
...
describe('toJUnit', () => {
  it('emits well-formed XML where only error failures are <failure>', () => {
    const run = makeRun({
      ...
      checks: [
        { id: 'manifest/event-kind', area: 'manifest', severity: 'error', title: 't', status: 'pass' },
        { id: 'boot/installs-global', area: 'boot', severity: 'error', title: 'g', status: 'fail', detail: 'no global' },
        { id: 'lifecycle/clean-teardown', area: 'lifecycle', severity: 'warning', title: 'd', status: 'fail', detail: 'leaked <listener>' },
      ],
    });
    ...
    expect(xml).toContain('&lt;listener&gt;'); // escaped
  });
