/**
 * @napplet/nap/connect -- Connect NAP module barrel.
 *
 ...
 *
 * NAP-CONNECT has NO wire protocol. The barrel registers a noop handler
 * only to ensure `dispatch.getRegisteredDomains()` includes `'connect'`.
 *
 * @example
 * ```ts
 * import type { NappletConnect } from '@napplet/nap/connect';
 * import { DOMAIN, installConnectShim, connectGranted } from '@napplet/nap/connect';
 * ```
 *
 ...
 * NAP-CONNECT has no wire protocol -- the handler is an intentional noop.
...
registerNap(DOMAIN, (_msg) => {
  /* NAP-CONNECT has no wire messages; grant flow is pure meta tag + CSP */
});
