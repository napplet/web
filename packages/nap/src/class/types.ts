/**
 * @napplet/nap/class -- Class NAP message types for the JSON envelope wire protocol.
 *
 * NAP-CLASS establishes the abstract napplet-class concept: shell assigns an
 ...
 *
 * Per NAP-CLASS.md: the integer value is a plain `number`, not a literal
 * union — the class space is extensible as new NAP-CLASS-$N sub-track
 * members are defined.
 */

import type { NappletMessage } from '@napplet/core';

/** The NAP domain name for class messages. */
export const DOMAIN = 'class' as const;

/**
 * Base interface for all class NAP messages.
 ...
 * `NAP-CLASS-$N` sub-track (e.g., `class: 1` -> `NAP-CLASS-1.md`).
 *
 * Normative constraints per NAP-CLASS.md:
...
export interface ClassAssignedMessage extends ClassMessage {
  ...
  id: string;
  /** Non-negative integer selecting a concrete NAP-CLASS-$N member. */
  class: number;
}
 ...
 * - When the shell does not implement `nap:class`.
 * - When the shell implements the NAP but intentionally withholds assignment.
 *
...
/** All class NAP message types (v1: single terminal assignment envelope). */
export type ClassNapMessage = ClassAssignedMessage;
